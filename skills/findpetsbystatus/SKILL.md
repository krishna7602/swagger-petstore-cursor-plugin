...
## When to use
Use this skill when the user intent maps to `findpetsbystatus` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `findpetsbystatus` and list required vs optional parameters.
...
4. Call the `findpetsbystatus` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `findpetsbystatus` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `findpetsbystatus`, extract the top-level summary fields.
